...
void NRF24L01_TxPacketMode2(u8 type,u8 *pbuf,u8 len)
{
	u8 fifo_sta;
	delay_ms(1);
	NRF24L01_CE = 0;
	NRF24L01_Write_Reg((WRITE_REG_NRF|0),0x0e);
	NRF24L01_CE = 1;
	delay_ms(1);
	NRF24L01_CE=0;
	...
	if((fifo_sta&0x20)==0)
	{
		NRF24L01_Write_Reg(0xE1,0);
		delay_ms(1);
		NRF24L01_Write_Buf(type, pbuf, len);
	}
	NRF24L01_CE = 1;
	delay_ms(1);
	NRF24L01_CE = 0;
	NRF24L01_Write_Reg((WRITE_REG_NRF|0),0x0f);
	NRF24L01_CE = 1;
}
...
void NRF24L01_RX_Mode(void)
{
	NRF24L01_CE=0;	  
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,0x0f);//设置TX发射参数,0db增益,2Mbps,低噪声增益开启   st
...
{														 
	NRF24L01_CE=0;	    
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+EN_AA,0x01);     //使能通道0的自动应答    
  	NRF24L01_Write_Reg(WRITE_REG_NRF+EN_RXADDR,0x01); //使能通道0的接收地址  
  	NRF24L01_Write_Reg(WRITE_REG_NRF+SETUP_RETR,0x1a);  // 0x1a);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
